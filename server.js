...
function sanitize(values, keys) { const item = {}; for (const key of keys) { const value = typeof values[key] === 'string' || typeof values[key] === 'number' ? repairTextEncoding(String(values[key]).trim()) : ''; if (!value || value.length > 250) return null; item[key] = value; } if (values.observacoes !== undefined) { const observacoes = repairTextEncoding(String(values.observacoes || '').trim()); if (observacoes.length > 5000) return null; item.observacoes = observacoes; } if (values.tipo === 'externa') { for (const key of ['empresa', 'contato', 'email', 'descricao']) { const value = repairTextEncoding(String(values[key] || '').trim()); if (!value || value.length > (key === 'descricao' ? 3000 : 250)) return null; item[key] = value; } if (!/^\S+@\S+\.\S+$/.test(item.email)) return null; item.tipo = 'externa'; } else if (keys === resourceDefinitions.demandas) item.tipo = 'interna'; return item; }
