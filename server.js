...
function hospitalOnly(user) { return user.permissions?.demandas?.scope === 'hospital'; }
...
const pgStore = {
  async users() { return (await pool.query('SELECT id, nome, email, perfil, active, permissions, salt, hash, must_change_password AS "mustChangePassword", created_at AS "createdAt" FROM users ORDER BY nome')).rows; },
  ...
  async findUserByLogin(login) { return (await pool.query('SELECT id, nome, email, perfil, active, permissions, salt, hash, must_change_password AS "mustChangePassword", created_at AS "createdAt" FROM users WHERE lower(email) = $1', [login])).rows[0]; },
...
async function initializePostgres() {
  ...
  await pool.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS must_change_password BOOLEAN NOT NULL DEFAULT false; ALTER TABLE users ADD COLUMN IF NOT EXISTS permissions JSONB; ALTER TABLE users ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT true');
...
async function api(req, res, url) {
  ...
  const match = pathname.match(/^\/api\/resources\/([a-z]+)(?:\/([\w-]+))?$/); if (match) {
    ...
    if (req.method === 'POST' && !recordId) {
      const body = await requestBody(req); if (resource === 'demandas' && hospitalOnly(user) && body.tipo !== 'externa') return error(res, 403, 'Este usuário pode abrir somente demandas hospitalares.'); const payload = sanitize(body, resourceDefinitions[resource]);
