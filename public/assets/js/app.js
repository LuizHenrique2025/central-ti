...
function recordsPage(resource) {
  ...
  const rows = state.records.filter(record => Object.values(record).join(' ').toLowerCase().includes(state.query.toLowerCase()) && (resource !== 'programas' || (!state.programStatus || record.status === state.programStatus) && (!state.programPeriodicity || record.periodicidade === state.programPeriodicity)) && filters.every(([key]) => !state.resourceFilters[`${resource}-${key}`] || record[key] === state.resourceFilters[`${resource}-${key}`]));
  const filterControls = filters.map(([key, label]) => { const values = [...new Set(state.records.map(record => record[key]).filter(Boolean))].sort((a, b) => String(a).localeCompare(String(b), 'pt-BR')); const selected = state.resourceFilters[`${resource}-${key}`] || ''; return `<select onchange="setResourceFilter('${resource}','${key}',this.value)"><option value="">${key === 'periodicidade' ? 'Mensal e anual' : `Todos os ${label.toLowerCase()}`}</option>${values.map(value => `<option ${selected === value ? 'selected' : ''}>${esc(value)}</option>`).join('')}</select>`; }).join('');
  ...
  return `${header(module.name, 'Gestão')}<div class="section-toolbar"><input class="search" value="${esc(state.query)}" oninput="setSearch(this.value)" placeholder="Pesquisar por patrimônio, IP, grupo ou responsável..."/>${resource === 'programas' ? `<select onchange="setProgramFilter('programStatus',this.value)"><option value="">Todos os status</option>${['Ativo','Em renovação','Cancelado'].map(value => `<option ${state.programStatus === value ? 'selected' : ''}>${value}</option>`).join('')}</select><select onchange="setProgramFilter('programPeriodicity',this.value)"><option value="">Mensal e anual</option>${['Mensal','Anual'].map(value => `<option ${state.programPeriodicity === value ? 'selected' : ''}>${value}</option>`).join('')}</select>` : ''}<div class="toolbar-actions"><button class="secondary" onclick="exportResource('${resource}')">⇩ Exportar CSV</button>${resource === 'computadores' && canUpdate(resource) ? `<button class="secondary" onclick="openGroupManager()">⚙ Grupos</button>` : ''}${canCreate(resource) ? `<button class="add-record" onclick="openRecord('${resource}')">+ Novo cadastro</button>` : ''}</div></div><div class="panel table-panel">${rows.length ? `<table class="data-table"><thead><tr>${module.fields.map(field => `<th>${field[1]}</th>`).join('')}${resource === 'computadores' ? '<th>Checklist</th>' : ''}<th>Ações</th></tr></thead><tbody>${rows.map(record => `<tr>${module.fields.map(field => `<td>${cell(record, field)}</td>`).join('')}${resource === 'computadores' ? `<td>${record.checklist?.length ? `✓ ${record.checklist.length}/${KIT.length}` : '—'}</td>` : ''}<td>${canUpdate(resource) ? `<button class="link-button" onclick="openRecord('${resource}','${record.id}')">Editar</button>` : ''}${canDelete(resource) ? ` <button class="danger-link" onclick="deleteRecord('${resource}','${record.id}')">Excluir</button>` : ''}</td></tr>`).join('')}</tbody></table>` : '<div class="empty">Nenhum registro encontrado.</div>'}</div>`;
}
function reportPage() { const report = state.report || { modules: [], alerts: [], demandStatus: [], total: 0 }; const materialCount = report.modules.find(item => item.resource === 'materiais')?.total || 0; return `${header('Relatórios', 'Visão consolidada')}<div class="report-actions"><label>De<input type="date" value="${esc(state.start)}" onchange="setPeriod(this.value,state.end)"/></label><label>Até<input type="date" value="${esc(state.end)}" onchange="setPeriod(state.start,this.value)"/></label><button class="secondary" onclick="exportReport()">⇩ Exportar CSV</button><button class="add-record" onclick="window.print()">🖨 Imprimir / PDF</button></div><section class="metrics">${[['Total de registros', report.total], ['Alertas técnicos', report.alerts.length], ['Materiais informados', materialCount], ['Demandas concluídas', report.demandStatus.find(item => item.status === 'Concluída')?.total || 0]].map(item => `<div class="metric"><div class="metric-name">${item[0]}</div><div class="metric-number">${item[1]}</div></div>`).join('')}</section><div class="report-grid"><section class="panel"><div class="panel-heading"><h3>Cadastros por módulo</h3></div><table class="data-table"><tbody>${report.modules.map(item => `<tr><td>${esc(modules[item.resource]?.name || item.resource)}</td><td><b>${item.total}</b></td></tr>`).join('')}</tbody></table></section><section class="panel"><div class="panel-heading"><h3>Demandas por status</h3></div><table class="data-table"><tbody>${report.demandStatus.map(item => `<tr><td>${tag(item.status)}</td><td><b>${item.total}</b></td></tr>`).join('')}</tbody></table></section><section class="panel"><div class="panel-heading"><h3>Alertas técnicos</h3></div>${report.alerts.length ? `<table class="data-table"><tbody>${report.alerts.map(item => `<tr><td>${esc(item.item)}</td><td>${tag(item.avaliacao)}</td></tr>`).join('')}</tbody></table>` : '<div class="empty">Sem alertas no período.</div>'}</section><section class="panel"><div class="panel-heading"><h3>Materiais disponíveis na T.I.</h3></div><div class="empty">Este módulo é informativo e não possui alertas ou controle de reposição.</div></section></div>`; }
