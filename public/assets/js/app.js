...
function emailPage() {
  ...
  return `${header('E-mail interno', 'Conversas entre usuários')}<section class="webmail"><aside class="mail-folders"><button class="compose-button" onclick="compose()">✎ <span>Escrever</span></button>${[['inbox', unread], ['sent', 0], ['trash', 0]].map(([folder, count]) => `<button class="folder ${state.mailFolder === folder ? 'active' : ''}" onclick="setMailFolder('${folder}')"><span>${icons[folder]} ${labels[folder]}</span><b>${count || ''}</b></button>`).join('')}<div class="mail-note">As respostas ficam agrupadas na mesma conversa.</div></aside><section class="mail-list panel"><div class="mail-list-tools"><input value="${esc(state.mailQuery)}" oninput="setMailSearch(this.value)" placeholder="Pesquisar conversas"/><button class="icon-btn" title="Atualizar" onclick="load()">↻</button></div><div class="mail-bulk-toolbar"><label><input type="checkbox" ${allSelected ? 'checked' : ''} onchange="toggleAllMailThreads(this.checked)"/> Selecionar todas</label><span>${selected.size ? `${selected.size} selecionada${selected.size === 1 ? '' : 's'}` : ''}</span><button class="icon-btn" title="Marcar como lida" ${selected.size ? '' : 'disabled'} onclick="markSelectedMailRead()">✓</button><button class="icon-btn" title="Mover para Apagadas" ${selected.size ? '' : 'disabled'} onclick="deleteSelectedMailThreads()">♲</button></div><div class="mail-list-title">${labels[state.mailFolder]}</div>${visibleThreads.map(thread => `<button class="mail-item ${thread.unreadCount ? 'unread' : ''}" onclick="readThread('${thread.id}')"><input class="mail-select" type="checkbox" ${selected.has(thread.id) ? 'checked' : ''} onclick="event.stopPropagation()" onchange="toggleMailThreadSelection('${thread.id}',this.checked)"/><span class="mail-avatar">${esc(thread.participant.nome.slice(0, 1).toUpperCase())}</span><span class="mail-item-content"><span class="mail-item-top"><b>${esc(thread.latest.sender.id === mine ? `Para: ${thread.participant.nome}` : thread.participant.nome)}</b><time>${formatDate(thread.latest.createdAt)}</time></span><span class="mail-subject">${esc(canonicalMailSubject(thread.latest.subject))}${thread.unreadCount ? ` <small class="mail-thread-count">${thread.unreadCount}</small>` : ''}</span><span class="mail-preview">${esc(thread.latest.body)}</span></span></button>`).join('') || '<div class="empty">Nenhuma conversa nesta caixa.</div>'}</section></section>`;
}
function usersPage() {
