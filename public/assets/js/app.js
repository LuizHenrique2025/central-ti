...
function mountDemandAttachmentViewer() { const record = state.modal?.type === 'demand-details' ? state.modal.record : null, target = document.querySelector('.ticket-description'); if (!record?.anexoPrint?.data || !['image/png', 'image/jpeg', 'image/webp'].includes(record.anexoPrint.mime) || !target || target.querySelector('.demand-attachment')) return; const section = document.createElement('section'); section.className = 'demand-attachment'; section.innerHTML = `<h3>Anexo</h3><img class="mail-screenshot" alt="Print anexado à demanda" src="data:${record.anexoPrint.mime};base64,${record.anexoPrint.data}"/>`; target.after(section); }
