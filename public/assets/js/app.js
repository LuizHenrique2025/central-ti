...
function enableScreenshotPaste() { document.querySelectorAll('form[onsubmit^="sendMail"],form.demand-modal').forEach(form => { if (form.dataset.screenshotPaste) return; const input = form.querySelector('[name="screenshot"],[name="demandScreenshot"]'); if (!input) return; form.dataset.screenshotPaste = 'true'; form.addEventListener('paste', event => { const file = [...(event.clipboardData?.files || [])].find(item => item.type.startsWith('image/')); if (!file) return; if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type) || file.size > 5_000_000) return toast('Cole somente um print PNG, JPG ou WEBP de até 5 MB.'); const transfer = new DataTransfer(); transfer.items.add(file); input.files = transfer.files; input.dispatchEvent(new Event('change')); event.preventDefault(); toast('Print colado no anexo.'); }); }); }
new MutationObserver(() => { mountMailScreenshotFields(); mountDemandScreenshotFields(); mountDemandAttachmentViewer(); enableScreenshotPaste(); }).observe(document.getElementById('app'), { childList: true, subtree: true });
