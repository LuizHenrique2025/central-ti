...
function demandCardControl(card) {
  const details = `<button type="button" class="card-details" data-action="demand-details" data-demand-id="${esc(card.id)}">Ver detalhes</button>`;
...
function demandRequesterFilter(records) { const requesters = [...new Set(records.map(record => String(record.solicitante || '').trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'pt-BR')); return `<select data-action="demand-requester-filter" aria-label="Filtrar demandas por solicitante"><option value="">Todos os usuários</option>${requesters.map(name => `<option value="${esc(name)}" ${state.demandRequester === name ? 'selected' : ''}>${esc(name)}</option>`).join('')}</select>`; }
function demandFilters(records) { const hasFilters = state.demandAssignee || state.demandRequester || state.demandCreatedDate; return `<span class="demand-filters">${demandRequesterFilter(records)}<label class="demand-date-filter">Data de abertura<input type="date" value="${esc(state.demandCreatedDate)}" data-action="demand-date-filter" aria-label="Filtrar demandas por data de abertura"/></label>${demandAssigneeFilter(records)}${hasFilters ? '<button class="secondary demand-filter-clear" data-action="clear-demand-filters">Limpar filtros</button>' : ''}</span>`; }
...
function demandPriorityRail(cards) { const now = Date.now(), nextDay = now + 24 * 60 * 60 * 1000; const active = cards.filter(card => !isCompletedDemand(card)); const groups = [['overdue', 'Atrasadas', active.filter(card => { const due = demandSlaTime(card); return due !== null && due < now; })], ['soon', 'Próximas do SLA', active.filter(card => { const due = demandSlaTime(card); return due !== null && due >= now && due <= nextDay; })], ['unassigned', 'Sem responsável', active.filter(card => !String(card.tecnicoResponsavel || '').trim())]]; const visible = groups.filter(([, , records]) => records.length); if (!visible.length) return '<section class="demand-priority-rail demand-priority-clear" aria-label="Prioridades de demandas">Sem prioridades críticas no momento.</section>'; const item = record => `<button type="button" class="demand-priority-item" data-action="demand-details" data-demand-id="${esc(record.id)}"><span>${esc(record.ticket || 'Demanda')}</span><b>${esc(record.titulo)}</b><small>${esc(record.tecnicoResponsavel || 'Sem responsável')}</small></button>`; return `<section class="demand-priority-rail" aria-label="Prioridades de demandas">${visible.map(([kind, label, records]) => `<section class="demand-priority-group priority-${kind}"><header><span>${esc(label)}</span><b>${records.length}</b></header><div>${records.slice(0, 3).map(item).join('')}${records.length > 3 ? `<small class="demand-priority-more">+${records.length - 3} demanda${records.length - 3 === 1 ? '' : 's'}</small>` : ''}</div></section>`).join('')}</section>`; }
function demandBoard() { const cards = state.records.filter(matchesDemandFilters); return `${header('Demandas', 'Quadro de trabalho')}${demandPriorityRail(cards)}<div class="section-toolbar"><input class="search" aria-label="Pesquisar demandas" value="${esc(state.query)}" data-action="search" placeholder="Pesquisar por ticket, demanda ou solicitante..."/>${demandFilters(state.records)}<div class="toolbar-actions">${canWrite('demandas') ? `<button class="secondary" data-action="status-manager">⚙ Status</button><button class="add-record" data-action="open-record" data-resource="demandas">+ Abrir chamado</button>` : ''}</div></div><div class="kanban">${state.statuses.map(status => { const statusCards = cards.filter(card => canonicalDemandStatus(card.status) === status); return `<section class="kanban-column" data-drop-status="${esc(status)}"><header><span class="kanban-dot ${demandStatusTone(status)}"></span><b>${esc(status)}</b><small>${statusCards.length}</small></header><div class="kanban-cards">${statusCards.map(card => `<article class="demand-card" ${canUpdate('demandas') ? `draggable="true" data-drag-demand-id="${esc(card.id)}"` : ''}><div class="demand-card-title">${esc(card.titulo)}</div><div class="demand-card-meta">${esc(card.solicitante)}</div><div class="demand-card-bottom">${tag(card.prioridade)}${demandCardControl(card)}</div></article>`).join('') || `<div class="kanban-empty">${demandEmptyState(cards, statusCards)}</div>`}</div></section>`; }).join('')}</div>`; }
