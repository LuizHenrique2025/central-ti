...
function demandReportFold(section, title, content, description = 'Clique para visualizar') { return `<details class="demand-report-fold" ${state.demandReportExpanded[section] ? 'open' : ''} ontoggle="setDemandReportExpanded('${section}',this.open)"><summary><span>${esc(title)}<small>${esc(description)}</small></span><b aria-hidden="true">⌄</b></summary>${content}</details>`; }
...
function demandReportsPage(demand) {
  ...
  const details = demandReportFold('details', 'Demandas detalhadas', `<section class="panel table-panel demand-report-table"><div class="panel-heading"><div><div class="panel-subtitle">${demand.records.length} registro(s) encontrados · página ${page} de ${pages}</div></div>${chooser}</div>${pageRows.length && visibleColumns.length ? `<table class="data-table"><thead><tr>${visibleColumns.map(([, label]) => `<th>${esc(label)}</th>`).join('')}</tr></thead><tbody>${pageRows.map(item => `<tr>${visibleColumns.map(([, , render]) => `<td>${render(item)}</td>`).join('')}</tr>`).join('')}</tbody></table>` : '<div class="empty">Sem demandas para os filtros ou nenhuma coluna selecionada.</div>'}<div class="report-pagination"><button class="secondary" type="button" ${page === 1 ? 'disabled' : ''} onclick="setDemandReportPage(${page - 1})">← Anterior</button><span>Página ${page} de ${pages}</span><button class="secondary" type="button" ${page === pages ? 'disabled' : ''} onclick="setDemandReportPage(${page + 1})">Próxima →</button></div></section>`, `${demand.records.length} registro(s) disponíveis`);
  const reasons = demandReportFold('reasons', 'Motivos mais frequentes', demandReportBars('', demand.reasons, false), `${demand.reasons.length} motivo(s) encontrado(s)`);
  const dates = demandReportFold('dates', 'Demandas abertas por data', demandReportBars('', demand.openedByDate, false), `${demand.openedByDate.length} data(s) encontrada(s)`);
  return `${printHeader}${demandReportFilters(demand)}<section class="metrics report-metrics">${[['Total de demandas', metrics.total], ['Concluídas', `${metrics.completed} (${metrics.completedPercent}%)`], ['Em aberto', metrics.open], ['Usuário com mais demandas', metrics.topRequester], ['Motivo mais frequente', metrics.topReason]].map(([label, value]) => `<div class="metric"><div class="metric-name">${esc(label)}</div><div class="metric-number">${esc(value)}</div></div>`).join('')}</section><div class="report-grid">${mainReasons}${professionals}</div><div class="report-grid">${reasons}${dates}</div>${details}${printFooter}`;
}
function reportPage() {
...
function setDemandReportExpanded(section, expanded) { state.demandReportExpanded[section] = Boolean(expanded); }
