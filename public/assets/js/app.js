...
function modal() { if (!state.modal) return ''; if (state.modal.type === 'demand-details') return demandDetailsModal(state.modal.record); if (state.modal.type === 'record') { const resource = state.modal.resource, record = state.modal.record, module = modules[resource]; if (resource === 'demandas') return demandModal(record); return `<div class="modal-backdrop" onclick="closeBack(event)"><form class="modal modal-wide" onsubmit="saveRecord(event,'${resource}','${record?.id || ''}')"><div class="modal-header"><h2>${record ? 'Editar' : 'Novo cadastro'} · ${module.name}</h2><button type="button" class="close" onclick="closeModal()">×</button></div>${resource === 'computadores' ? '<p class="modal-intro">Cadastre o IP para localizar a máquina na rede. As datas são opcionais e registram o ciclo de solicitação, entrega e devolução.</p>' : ''}<div class="two-col">${module.fields.map(item => field(item, record)).join('')}</div>${resource === 'computadores' ? `<fieldset class="checklist"><legend>Checklist do kit entregue</legend><div class="checklist-grid">${KIT.map(item => `<label class="check-item"><input type="checkbox" name="checklist" value="${item}" ${(record?.checklist || []).includes(item) ? 'checked' : ''}/><span>✓</span>${item}</label>`).join('')}</div></fieldset>` : ''}<div class="modal-actions"><button type="button" class="secondary" onclick="closeModal()">Cancelar</button><button class="primary">Salvar</button></div></form></div>`; }
  ...
  if (state.modal.type === 'mail-thread') {
    ...
    return `<div class="modal-backdrop" onclick="closeBack(event)"><section class="modal modal-wide mail-thread-modal"><div class="modal-header mail-thread-header"><div class="mail-thread-person"><span class="mail-thread-avatar">${esc(correspondent.nome.slice(0, 1).toUpperCase())}</span><div><h2>${esc(canonicalMailSubject(latest.subject))}</h2><p class="modal-intro">${esc(correspondent.nome)} · ${messages.length} mensagem${messages.length === 1 ? '' : 'ens'}</p></div></div><button type="button" class="close" aria-label="Fechar conversa" onclick="closeModal(true)">×</button></div><div class="mail-thread-history" aria-label="Histórico da conversa">${messages.map(message => `<article class="mail-thread-message ${message.sender.id === mine ? 'mine' : ''}"><header><span><b>${esc(message.sender.id === mine ? 'Você' : message.sender.nome)}</b>${message.sender.id === mine ? '<small>Enviado por você</small>' : ''}</span><time>${formatDate(message.createdAt)}</time></header><p>${esc(message.body).replace(/\n/g, '<br/>')}</p></article>`).join('')}</div>${isTrash ? '' : `<form class="mail-thread-reply" onsubmit="sendMail(event,'${latest.id}','${state.modal.threadId}')"><label class="field">Responder<textarea name="body" required rows="3" maxlength="5000" autofocus placeholder="Escreva sua resposta..."></textarea></label><div class="modal-actions"><button type="button" class="secondary" onclick="closeModal(true)">Fechar</button><button class="primary">Responder</button></div></form>`}</section></div>`;
  }
...
async function sendMail(event, replyToId = '', currentThreadId = '') { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); if (replyToId) data.replyToId = replyToId; try { await api('/api/messages', { method: 'POST', body: JSON.stringify(data) }); if (currentThreadId) { const mail = await api('/api/messages'); state.messages = mail.messages; state.unreadMessages = unreadCount(state.messages); state.modal = { type: 'mail-thread', threadId: currentThreadId }; state.formDirty = false; render({ preserveScroll: true }); } else { await closeModal(true); await load(); } toast('Mensagem enviada.'); } catch (error) { toast(error.message); } }
