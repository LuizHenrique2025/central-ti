...
async function savePreRegistrationCorrection(event, id) { event.preventDefault(); try { await api(`/api/users/${id}/pre-cadastro`, { method: 'PUT', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); closeModal(true); await load(); toast('Dados de validação corrigidos.'); } catch (error) { toast(error.message); } }
