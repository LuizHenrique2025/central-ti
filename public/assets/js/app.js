...
function emailPage() {
  ...
  const visibleThreads = mailThreads(folders[state.mailFolder], mine).map(thread => {
    ...
    return { ...thread, allMessages, latest, participant, unread: allMessages.some(message => message.recipient.id === mine && !message.readAt) };
  ...
  return `${header('E-mail interno', 'Conversas entre usuários')}<section class="webmail"><aside class="mail-folders"><button class="compose-button" onclick="compose()">✎ <span>Escrever</span></button>${[['inbox', unread], ['sent', mailThreads(folders.sent, mine).length], ['trash', mailThreads(folders.trash, mine).length]].map(([folder, count]) => `<button class="folder ${state.mailFolder === folder ? 'active' : ''}" onclick="setMailFolder('${folder}')"><span>${icons[folder]} ${labels[folder]}</span><b>${count || ''}</b></button>`).join('')}<div class="mail-note">As respostas ficam agrupadas na mesma conversa.</div></aside><section class="mail-list panel"><div class="mail-list-tools"><input value="${esc(state.mailQuery)}" oninput="setMailSearch(this.value)" placeholder="Pesquisar conversas"/><button class="icon-btn" title="Atualizar" onclick="load()">↻</button></div><div class="mail-list-title">${labels[state.mailFolder]} <span>${visibleThreads.length} conversa${visibleThreads.length === 1 ? '' : 's'}</span></div>${visibleThreads.map(thread => `<button class="mail-item ${thread.unread ? 'unread' : ''}" onclick="readThread('${thread.id}')"><span class="mail-avatar">${esc(thread.participant.nome.slice(0, 1).toUpperCase())}</span><span class="mail-item-content"><span class="mail-item-top"><b>${esc(thread.latest.sender.id === mine ? `Para: ${thread.participant.nome}` : thread.participant.nome)}</b><time>${formatDate(thread.latest.createdAt)}</time></span><span class="mail-subject">${esc(canonicalMailSubject(thread.latest.subject))}${thread.allMessages.length > 1 ? ` <small class="mail-thread-count">${thread.allMessages.length}</small>` : ''}</span><span class="mail-preview">${esc(thread.latest.body)}</span></span></button>`).join('') || '<div class="empty">Nenhuma conversa nesta caixa.</div>'}</section></section>`;
}
function usersPage() {
...
function modal() { if (!state.modal) return ''; if (state.modal.type === 'demand-details') return demandDetailsModal(state.modal.record); if (state.modal.type === 'record') { const resource = state.modal.resource, record = state.modal.record, module = modules[resource]; if (resource === 'demandas') return demandModal(record); return `<div class="modal-backdrop" onclick="closeBack(event)"><form class="modal modal-wide" onsubmit="saveRecord(event,'${resource}','${record?.id || ''}')"><div class="modal-header"><h2>${record ? 'Editar' : 'Novo cadastro'} · ${module.name}</h2><button type="button" class="close" onclick="closeModal()">×</button></div>${resource === 'computadores' ? '<p class="modal-intro">Cadastre o IP para localizar a máquina na rede. As datas são opcionais e registram o ciclo de solicitação, entrega e devolução.</p>' : ''}<div class="two-col">${module.fields.map(item => field(item, record)).join('')}</div>${resource === 'computadores' ? `<fieldset class="checklist"><legend>Checklist do kit entregue</legend><div class="checklist-grid">${KIT.map(item => `<label class="check-item"><input type="checkbox" name="checklist" value="${item}" ${(record?.checklist || []).includes(item) ? 'checked' : ''}/><span>✓</span>${item}</label>`).join('')}</div></fieldset>` : ''}<div class="modal-actions"><button type="button" class="secondary" onclick="closeModal()">Cancelar</button><button class="primary">Salvar</button></div></form></div>`; }
  ...
  if (state.modal.type === 'mail-thread') {
    ...
    return `<div class="modal-backdrop" onclick="closeBack(event)"><section class="modal modal-wide mail-thread-modal"><div class="modal-header"><div><h2>${esc(canonicalMailSubject(latest.subject))}</h2><p class="modal-intro">${messages.length} mensagem${messages.length === 1 ? '' : 'ens'} nesta conversa</p></div><button type="button" class="close" onclick="closeModal(true)">×</button></div><div class="mail-thread-history">${messages.map(message => `<article class="mail-thread-message ${message.sender.id === mine ? 'mine' : ''}"><header><b>${esc(message.sender.id === mine ? 'Você' : message.sender.nome)}</b><time>${formatDate(message.createdAt)}</time></header><p>${esc(message.body).replace(/\n/g, '<br/>')}</p></article>`).join('')}</div>${isTrash ? '' : `<form class="mail-thread-reply" onsubmit="sendMail(event,'${latest.id}','${state.modal.threadId}')"><label class="field">Responder<textarea name="body" required rows="4" maxlength="5000" autofocus placeholder="Escreva sua resposta..."></textarea></label><div class="modal-actions"><button type="button" class="secondary" onclick="closeModal(true)">Fechar</button><button class="primary">Responder</button></div></form>`}</section></div>`;
  }
...
async function sendMail(event, replyToId = '', currentThreadId = '') { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); if (replyToId) data.replyToId = replyToId; try { await api('/api/messages', { method: 'POST', body: JSON.stringify(data) }); await load(); if (currentThreadId) { state.modal = { type: 'mail-thread', threadId: currentThreadId }; state.formDirty = false; render(); } else await closeModal(true); toast('Mensagem enviada.'); } catch (error) { toast(error.message); } }
