...
function demandReportFilters(report) {
  ...
  return `<section class="report-filter-grid demand-report-filters">${fields.map(([key, label, options]) => `<label>${label}<select multiple size="3" aria-label="Filtrar demandas por ${label.toLowerCase()}" onchange="setDemandReportFilter('${key}',[...this.selectedOptions].map(option=>option.value))">${options.map(option => `<option value="${esc(option)}" ${report.filters[key].includes(option) ? 'selected' : ''}>${esc(option)}</option>`).join('')}</select></label>`).join('')}</section>`;
}
...
function demandReportsPage(demand) {
  ...
  const details = demandReportTable('Demandas detalhadas', ['Ticket', 'Abertura', 'Solicitante', 'Setor', 'Motivo', 'Responsável', 'Status'], demand.records.map(item => `<tr><td>${esc(item.ticket)}</td><td>${item.createdAt ? esc(formatDate(item.createdAt)) : '—'}</td><td>${esc(item.requester)}</td><td>${esc(item.sector)}</td><td>${esc(item.reason)}</td><td>${esc(item.assignee)}</td><td>${tag(item.status)}</td></tr>`));
  return `${demandReportFilters(demand)}<section class="metrics report-metrics">${[['Total de demandas', metrics.total], ['Concluídas', `${metrics.completed} (${metrics.completedPercent}%)`], ['Em aberto', metrics.open], ['Usuário com mais demandas', metrics.topRequester], ['Motivo mais frequente', metrics.topReason]].map(([label, value]) => `<div class="metric"><div class="metric-name">${esc(label)}</div><div class="metric-number">${esc(value)}</div></div>`).join('')}</section><div class="report-grid">${mainReasons}${professionals}</div><div class="report-grid">${demandReportBars('Motivos mais frequentes', demand.reasons)}${demandReportBars('Demandas abertas por data', demand.openedByDate)}</div>${details}`;
}
function reportPage() {
...
function setDemandReportFilter(field, values) { state.demandReportFilters[field] = Array.isArray(values) ? values : []; load(); }
