...
function demandReportBars(title, rows) {
  return demandReportTable(title, ['Descrição', 'Quantidade'], rows.map(item => `<tr><td>${esc(item.label)}</td><td>${item.total}</td></tr>`));
}
function demandReportsPage(demand) {
  ...
  const reportHeader = `<header class="demand-report-header"><div class="demand-report-brand">CENTRAL T.I.<span>GESTÃO DE TECNOLOGIA</span></div><div class="demand-report-title"><strong>RELATÓRIO OPERACIONAL DE DEMANDAS</strong><span>Consulta consolidada conforme os parâmetros informados</span></div><div class="demand-report-meta"><span>PERÍODO</span><b>${esc(period)}</b><span>EMITIDO EM</span><b>${esc(formatDate(new Date().toISOString()))}</b></div></header>`;
  const summary = `<section class="demand-report-summary"><div><span>Total de demandas</span><b>${metrics.total}</b></div><div><span>Concluídas</span><b>${metrics.completed} (${metrics.completedPercent}%)</b></div><div><span>Em aberto</span><b>${metrics.open}</b></div><div><span>Maior solicitante</span><b>${esc(metrics.topRequester)}</b></div><div><span>Motivo recorrente</span><b>${esc(metrics.topReason)}</b></div></section>`;
  ...
  return `${demandReportFilters(demand)}<article class="demand-report-document">${reportHeader}${summary}<div class="demand-report-analysis">${mainReasons}${professionals}${demandReportBars('Motivos mais frequentes', demand.reasons)}${demandReportBars('Demandas por data de abertura', demand.openedByDate)}</div>${details}<footer class="demand-report-footer">Central TI · Relatório de uso interno · Dados obtidos conforme os parâmetros selecionados.</footer></article>`;
}
function reportPage() {
...
function printDemandReport() {
  ...
  const summary = `<section class="summary"><table><tbody><tr>${metrics.map(([label, value]) => `<td><span>${esc(label)}</span><b>${esc(value)}</b></td>`).join('')}</tr></tbody></table></section>`;
  const mainReasons = table('Principal motivo por solicitante', ['Solicitante', 'Setor', 'Total', 'Principal motivo', 'Qtd.', '% no usuário', '% geral'], demand.mainReasons.map(item => `<tr><td>${esc(item.requester)}</td><td>${esc(item.sector)}</td><td>${item.total}</td><td>${esc(item.reason)}</td><td>${item.quantity}</td><td>${item.percentOfRequester}%</td><td>${item.percentOfTotal}%</td></tr>`));
  ...
  printWindow.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Relatório de Demandas — Central TI</title><style>@page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#202428;font:9pt Arial,sans-serif}.header{display:grid;grid-template-columns:1fr 1.8fr 1fr;gap:6mm;align-items:center;border-bottom:2px solid #3d444a;padding-bottom:6mm;margin-bottom:6mm}.brand{font-size:17pt;font-weight:800;letter-spacing:-.6pt}.brand span{display:block;margin-top:1mm;color:#667078;font-size:7pt;letter-spacing:.7pt}.title{text-align:center}.title b{display:block;font-size:11pt;letter-spacing:.15pt}.title span{display:block;margin-top:1.4mm;color:#626a71;font-size:8pt}.meta{display:grid;grid-template-columns:auto 1fr;gap:1.2mm 3mm;align-items:baseline;font-size:7pt;text-align:right}.meta span{color:#6a7279;font-weight:700}.meta b{font-size:8pt}.summary{margin:0 0 6mm}.summary table{border:1px solid #cfd4d8;background:#f3f4f5}.summary td{width:20%;padding:3mm;border:0;border-right:1px solid #cfd4d8}.summary td:last-child{border-right:0}.summary span{display:block;color:#646c73;font-size:7pt;font-weight:700;text-transform:uppercase}.summary b{display:block;margin-top:1.5mm;font-size:10pt}section{margin:0 0 6mm;break-inside:avoid}h2{margin:0;padding:2.5mm 3mm;border:1px solid #cbd0d4;border-bottom:0;background:#e5e7e9;color:#273038;font-size:9pt;letter-spacing:.2pt;text-transform:uppercase}table{width:100%;border-collapse:collapse;font-size:7.5pt}th{background:#f1f2f3;color:#373f46;text-align:left;font-size:7pt;font-weight:700}th,td{border:1px solid #d2d6d9;padding:2.1mm;vertical-align:top}tr{break-inside:avoid}thead{display:table-header-group}.footer{margin-top:8mm;padding-top:3mm;border-top:1px solid #cbd0d4;color:#70777d;font-size:7pt;text-align:center}</style></head><body><header class="header"><div class="brand">CENTRAL T.I.<span>GESTÃO DE TECNOLOGIA</span></div><div class="title"><b>RELATÓRIO OPERACIONAL DE DEMANDAS</b><span>Consulta consolidada conforme os parâmetros informados</span></div><div class="meta"><span>PERÍODO</span><b>${esc(period)}</b><span>EMITIDO POR</span><b>${esc(generatedBy)}</b><span>EMITIDO EM</span><b>${esc(formatDate(new Date().toISOString()))}</b></div></header>${summary}${mainReasons}${professionals}${details}<footer class="footer">Central TI · Relatório de uso interno · Dados obtidos conforme os parâmetros selecionados.</footer><script>window.onload=()=>window.print();<\/script></body></html>`);
