...
state.demandReportSection ||= 'professionals';
...
function demandCardControl(card) {
  const details = `<button type="button" class="card-details" data-action="demand-details" data-demand-id="${esc(card.id)}">Abrir demanda</button>`;
  if (!canUpdate('demandas')) return details;
  const status = `<select aria-label="Alterar status de ${esc(card.ticket || card.titulo)}" data-action="move-demand" data-demand-id="${esc(card.id)}">${state.statuses.map(option => `<option ${option === card.status ? 'selected' : ''}>${esc(option)}</option>`).join('')}</select>`;
  return `<span class="demand-card-actions">${details}${status}</span>`;
}
function demandEmptyState(cards, statusCards) {
  const hasFilters = Boolean(state.query || state.demandAssignee || state.demandRequester || state.demandCreatedDate);
  if (hasFilters && !cards.length) return 'Nenhuma demanda corresponde aos filtros aplicados.';
...
function demandCreatedDate(record) { const date = new Date(record.createdAt); if (Number.isNaN(date.getTime())) return ''; return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`; }
function matchesDemandRequester(record) { return !state.demandRequester || String(record.solicitante || '').trim() === state.demandRequester; }
function matchesDemandCreatedDate(record) { return !state.demandCreatedDate || demandCreatedDate(record) === state.demandCreatedDate; }
...
function recordsPage(resource) {
  ...
  const placeholder = resource === 'pedidos' ? 'Pesquisar por material, setor ou solicitante...' : resource === 'ramais' ? 'Pesquisar por ramal, setor, responsável ou e-mail...' : 'Pesquisar por patrimônio, IP, grupo ou responsável...';
  return `${header(module.name, resource === 'pedidos' ? 'Solicite materiais e acompanhe a entrega' : 'Gestão')}<div class="section-toolbar"><input class="search" value="${esc(state.query)}" oninput="setSearch(this.value)" placeholder="${placeholder}"/>${resource === 'programas' ? `<select onchange="setProgramFilter('programStatus',this.value)"><option value="">Todos os status</option>${['Ativo','Em renovação','Cancelado'].map(value => `<option ${state.programStatus === value ? 'selected' : ''}>${value}</option>`).join('')}</select><select onchange="setProgramFilter('programPeriodicity',this.value)"><option value="">Mensal e anual</option>${['Mensal','Anual'].map(value => `<option ${state.programPeriodicity === value ? 'selected' : ''}>${value}</option>`).join('')}</select>` : ''}<div class="toolbar-actions"><button class="secondary" onclick="exportResource('${resource}')">⇩ Exportar CSV</button>${resource === 'computadores' && canUpdate(resource) ? `<button class="secondary" onclick="openGroupManager()">⚙ Grupos</button>` : ''}${canCreate(resource) ? `<button class="add-record" onclick="openRecord('${resource}')">${resource === 'pedidos' ? '+ Solicitar material' : '+ Novo cadastro'}</button>` : ''}</div></div><div class="panel table-panel">${rows.length ? `<table class="data-table"><thead><tr>${columns}${resource === 'computadores' ? '<th>Checklist</th>' : ''}<th>Ações</th></tr></thead><tbody>${rows.map(record => `<tr>${module.fields.map(field => `<td>${cell(record, field)}</td>`).join('')}${resource === 'computadores' ? `<td>${record.checklist?.length ? `✓ ${record.checklist.length}/${KIT.length}` : '—'}</td>` : ''}<td>${resource === 'redes' && canRead(resource) ? `<button class="link-button" data-action="network-qr" data-network-id="${esc(record.id)}">QR Wi-Fi</button>` : ''}${canUpdate(resource) ? `<button class="link-button" onclick="openRecord('${resource}','${record.id}')">Editar</button>` : ''}${canDelete(resource) ? ` <button class="danger-link" onclick="deleteRecord('${resource}','${record.id}')">Excluir</button>` : ''}</td></tr>`).join('')}</tbody></table>` : '<div class="empty">Nenhum registro encontrado.</div>'}</div>`;
}
...
function demandReportTable(title, headers, rows, showHeading = true) {
  return `<section class="panel table-panel demand-report-table">${showHeading ? `<div class="panel-heading"><h3>${esc(title)}</h3></div>` : ''}${rows.length ? `<table class="data-table"><thead><tr>${headers.map(item => `<th>${esc(item)}</th>`).join('')}</tr></thead><tbody>${rows.join('')}</tbody></table>` : '<div class="empty">Sem dados no período.</div>'}</section>`;
}
function demandReportBars(title, rows, showHeading = true) {
  const maximum = Math.max(1, ...rows.map(item => item.total));
  return `<section class="panel exclusion-chart">${showHeading ? `<div class="panel-heading"><h3>${esc(title)}</h3></div>` : ''}${rows.length ? `<div class="bar-list">${rows.map(item => `<div class="bar-row"><span>${esc(item.label)}</span><div><i style="width:${Math.max(4, Math.round(item.total / maximum * 100))}%"></i></div><b>${item.total}</b></div>`).join('')}</div>` : '<div class="empty">Sem dados no período.</div>'}</section>`;
}
function demandReportsPage(demand) {
  ...
  const sections = { professionals, mainReasons, reasons: demandReportBars('Motivos mais frequentes', demand.reasons), dates: demandReportBars('Demandas abertas por data', demand.openedByDate), details };
  const sectionTabs = [['professionals', 'Distribuição por profissional'], ['mainReasons', 'Principal motivo por solicitante'], ['reasons', 'Motivos mais frequentes'], ['dates', 'Demandas abertas por data'], ['details', 'Demandas detalhadas']];
  return `${printHeader}${demandReportFilters(demand)}<section class="metrics report-metrics">${[['Total de demandas', metrics.total], ['Concluídas', `${metrics.completed} (${metrics.completedPercent}%)`], ['Em aberto', metrics.open], ['Usuário com mais demandas', metrics.topRequester], ['Motivo mais frequente', metrics.topReason]].map(([label, value]) => `<div class="metric"><div class="metric-name">${esc(label)}</div><div class="metric-number">${esc(value)}</div></div>`).join('')}</section><nav class="report-tabs demand-analysis-tabs" aria-label="Análises de demandas">${sectionTabs.map(([id, label]) => `<button class="${state.demandReportSection === id ? 'active' : ''}" type="button" onclick="setDemandReportSection('${id}')">${esc(label)}</button>`).join('')}</nav>${sections[state.demandReportSection] || professionals}${printFooter}`;
}
function reportPage() {
...
function usersPage() {
  ...
  const canReopenFirstAccess = user => user.id !== state.user.id && user.active === true && !String(user.email || '').trim() && !String(user.login || '').trim() && user.cpfLast4;
  const canCorrectPreRegistration = user => user.id !== state.user.id && user.active === false && user.activationStatus === 'pre-cadastro' && user.cpfLast4;
  const actions = user => `${user.activationStatus === 'aguardando aprovação' ? `<button class="add-record" onclick="approveUser('${user.id}')">Ativar cadastro</button> ` : ''}${user.id === state.user.id ? '<span class="muted">Seu usuário</span>' : `${canCorrectPreRegistration(user) ? `<button class="link-button" onclick="openPreRegistrationCorrection('${user.id}')">Corrigir pré-cadastro</button> ` : ''}${canReopenFirstAccess(user) ? `<button class="link-button" onclick="reopenFirstAccess('${user.id}')">Reabrir primeiro acesso</button> ` : ''}<button class="link-button" onclick="openPasswordReset('${user.id}')">Redefinir senha</button> <button class="link-button" onclick="openPermissions('${user.id}')">Permissões</button> <button class="${user.active === false ? 'link-button' : 'danger-link'}" onclick="setUserActive('${user.id}',${user.active === false})">${user.active === false ? 'Ativar' : 'Desativar'}</button>`}`;
...
function materialRequestField(key, label, record) {
  const value = record?.[key] ?? '';
  if (key === 'categoria') return '';
  if (key === 'solicitante') return `<label class="field">${label}<input value="${esc(record?.solicitante || state.user.nome)}" readonly/></label>`;
  if (key === 'item') return `<label class="field">${label}<select name="item" required><option value="">Selecione o material</option>${Object.entries(MATERIAL_CATALOG).map(([category, items]) => `<optgroup label="${category}">${items.map(item => `<option ${item === value ? 'selected' : ''}>${esc(item)}</option>`).join('')}</optgroup>`).join('')}</select></label>`;
  if (key === 'quantidade') return `<label class="field">${label}<input name="quantidade" type="number" min="1" max="10000" step="1" required value="${esc(value || 1)}"/><small>Bobinas e periféricos: unidades. Papel A4: resmas. Lápis: unidades.</small></label>`;
  if (key === 'status') return record && canUpdate('pedidos') ? `<label class="field">${label}<select name="status">${MATERIAL_STATUSES.map(status => `<option ${status === value ? 'selected' : ''}>${status}</option>`).join('')}</select></label>` : '<p class="modal-intro">O pedido será registrado como <b>Solicitado</b>.</p>';
  if (key === 'observacoes') return `<label class="field">${label}<textarea name="observacoes" maxlength="1000" rows="3" placeholder="Para materiais diversos, descreva o item e a unidade desejada.">${esc(value)}</textarea></label>`;
  return `<label class="field">${label}<input name="${key}" required maxlength="120" value="${esc(value)}" placeholder="Informe o setor que receberá o material"/></label>`;
}
function field([key, label, source], record) {
  if (state.modal?.resource === 'pedidos') return materialRequestField(key, label, record);
...
function setupModalAccessibility() {
  ...
  if (!dialog) {
    if (modalOpener?.isConnected) requestAnimationFrame(() => modalOpener.focus());
    modalOpener = null;
    return;
  }
  ...
  requestAnimationFrame(() => (dialog.querySelector('[autofocus], input:not([type="hidden"]):not([readonly]), select, textarea, button') || dialog).focus());
}
function modalFocusable(dialog) { return [...dialog.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]):not([type="hidden"]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])')].filter(element => !element.hidden && getComputedStyle(element).display !== 'none' && getComputedStyle(element).visibility !== 'hidden'); }
document.addEventListener('keydown', event => {
  const dialog = document.querySelector('.system-confirmation-card') || document.querySelector('.modal-backdrop .modal');
  if (!dialog) return;
  if (event.key === 'Escape') { event.preventDefault(); if (dialog.classList.contains('system-confirmation-card')) dialog.querySelector('.secondary')?.click(); else closeModal(); return; }
...
function setDemandReportSection(section) { state.demandReportSection = section; render({ preserveScroll: true }); }
...
function addObservationField() {
  if (state.modal?.resource === 'pedidos') return;
...
function permissionTable(current) { const actions = [['list','Listar'],['create','Incluir'],['update','Alterar'],['consult','Consultar'],['delete','Excluir']]; return `<fieldset class="permission-table"><legend>Permissões por módulo</legend><p class="modal-intro">Marque somente os módulos e ações que este usuário poderá utilizar. Módulos sem permissão não aparecem para ele.</p><p class="modal-intro">Materiais: todos podem solicitar e consultar os próprios pedidos. A T.I. acompanha todos e atualiza o atendimento; somente administradores excluem pedidos.</p><div class="permission-scroll"><table><thead><tr><th>Módulo</th><th>Todos</th>${actions.map(([,label]) => `<th>${label}</th>`).join('')}</tr></thead><tbody>${Object.entries(modules).filter(([id]) => id !== 'pedidos' && isModuleEnabled(id)).map(([id, module]) => { const value = current[id]; const legacy = value ? value.read !== false : false; const checked = action => { if (!value) return false; return action === 'list' || action === 'consult' ? value[action] ?? legacy : value[action] ?? value.write ?? false; }; return `<tr><td><b>${esc(module.name)}</b></td><td><input type="checkbox" class="permission-all" data-resource="${id}" onchange="toggleAllPermissions('${id}',this.checked)" ${actions.every(([action]) => checked(action)) ? 'checked' : ''}/></td>${actions.map(([action]) => `<td><input type="checkbox" name="permission-${id}-${action}" ${checked(action) ? 'checked' : ''} onchange="syncPermissionAll('${id}')"/></td>`).join('')}</tr>`; }).join('')}</tbody></table></div></fieldset>`; }
function openUser() { state.modal = { type: 'user' }; render(); }
function openPreRegistration() { state.modal = { type: 'pre-registration' }; render(); }
function openPreRegistrationCorrection(id) { const user = state.users.find(item => item.id === id); if (!user) return; state.modal = { type: 'pre-registration-correction', user }; render(); }
...
async function saveAnnouncement(event) { event.preventDefault(); try { await api('/api/announcements', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); closeModal(true); await load(); toast('Comunicado publicado para todos.'); } catch (error) { showFormError(event.target, error.message); } }
...
function showFormError(form, message) {
  ...
  if (!error) { error = document.createElement('p'); error.dataset.formError = 'true'; error.className = 'form-error'; error.tabIndex = -1; error.setAttribute('role', 'alert'); (form.querySelector('.modal-actions') || form.querySelector('button[type="submit"], button:not([type])'))?.before(error); }
  error.textContent = message || 'Não foi possível concluir a operação. Revise os dados e tente novamente.';
  error.focus();
}
async function saveRecord(event, resource, id) { event.preventDefault(); const form = event.target, formData = new FormData(form), data = Object.fromEntries(formData); form.querySelector('[data-form-error]')?.remove(); delete data.demandScreenshot; const screenshot = form.querySelector('[name="demandScreenshot"]')?.files[0]; if (screenshot) { if (!['image/png', 'image/jpeg', 'image/webp'].includes(screenshot.type) || screenshot.size > 5_000_000) return showFormError(form, 'Envie somente um print PNG, JPG ou WEBP de até 5 MB.'); const base64 = await new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(',')[1]); reader.onerror = reject; reader.readAsDataURL(screenshot); }); data.anexoPrint = { mime: screenshot.type, data: base64 }; } const observation = form.querySelector('[name="novaObservacao"]'); if (observation) data.novaObservacao = observation.value.trim(); if (resource === 'computadores') data.checklist = formData.getAll('checklist'); try { await api(`/api/resources/${resource}${id ? `/${id}` : ''}`, { method: id ? 'PUT' : 'POST', body: JSON.stringify(data) }); closeModal(true); if (state.page.startsWith('demandas-')) await go(state.page); else await load(); toast(resource === 'pedidos' ? 'Solicitação salva.' : 'Cadastro salvo.'); } catch (error) { showFormError(form, error.message); } }
...
async function dropDemand(event, status) { event.preventDefault(); event.stopPropagation(); const demandId = draggedDemandId || event.dataTransfer.getData('demandId'); draggedDemandId = null; stopKanbanAutoScroll(); if (!demandId || !state.records.some(record => record.id === demandId)) return; await moveDemand(demandId, status); }
...
async function saveUser(event) { event.preventDefault(); const form = new FormData(event.target), data = Object.fromEntries(form); data.permissions = collectPermissions(form); try { await api('/api/users', { method: 'POST', body: JSON.stringify(data) }); closeModal(true); await load(); toast('Usuário criado.'); } catch (error) { showFormError(event.target, error.message); } }
async function savePreRegistration(event) { event.preventDefault(); try { await api('/api/users/pre-cadastro', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); closeModal(true); await load(); toast('Pré-cadastro criado.'); } catch (error) { showFormError(event.target, error.message); } }
async function savePreRegistrationCorrection(event, id) { event.preventDefault(); try { await api(`/api/users/${id}/pre-cadastro`, { method: 'PUT', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); closeModal(true); await load(); toast('Dados de validação corrigidos.'); } catch (error) { showFormError(event.target, error.message); } }
async function savePermissions(event, id) { event.preventDefault(); try { await api(`/api/users/${id}/permissions`, { method: 'PUT', body: JSON.stringify({ permissions: collectPermissions(new FormData(event.target)) }) }); closeModal(true); await load(); toast('Permissões atualizadas.'); } catch (error) { showFormError(event.target, error.message); } }
...
async function reopenFirstAccess(id) { if (!await confirmDialog({ title: 'Reabrir primeiro acesso', message: 'O cadastro voltará ao pré-cadastro. A pessoa deverá informar e-mail, login e senha novamente.', confirmLabel: 'Reabrir primeiro acesso' })) return; try { await api(`/api/users/${id}/reopen-first-access`, { method: 'POST', body: '{}' }); await load(); toast('Primeiro acesso reaberto.'); } catch (error) { toast(error.message); } }
