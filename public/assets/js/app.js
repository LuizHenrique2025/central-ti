...
function mountMailScreenshotFields() { document.querySelectorAll('form[onsubmit^="sendMail"]').forEach(form => { if (form.querySelector('[name="screenshot"]')) return; const field = document.createElement('label'); field.className = 'field mail-screenshot-field'; field.innerHTML = '<span>Adicionar print <small>PNG, JPG ou WEBP · até 5 MB</small></span><input name="screenshot" type="file" accept="image/png,image/jpeg,image/webp"/><small class="mail-screenshot-preview"></small>'; form.querySelector('.modal-actions')?.before(field); field.querySelector('input').addEventListener('change', event => { const file = event.target.files[0], preview = field.querySelector('.mail-screenshot-preview'); preview.textContent = file ? `${file.name} · ${(file.size / 1024 / 1024).toFixed(2)} MB` : ''; }); }); }
function mountDemandScreenshotFields() { document.querySelectorAll('form.demand-modal').forEach(form => { if (form.querySelector('[name="demandScreenshot"]')) return; const field = document.createElement('label'); field.className = 'field mail-screenshot-field'; field.innerHTML = '<span>Anexo · adicionar print <small>PNG, JPG ou WEBP · até 5 MB</small></span><input name="demandScreenshot" type="file" accept="image/png,image/jpeg,image/webp"/><small class="mail-screenshot-preview"></small>'; form.querySelector('.modal-actions')?.before(field); field.querySelector('input').addEventListener('change', event => { const file = event.target.files[0], preview = field.querySelector('.mail-screenshot-preview'); preview.textContent = file ? `${file.name} · ${(file.size / 1024 / 1024).toFixed(2)} MB` : ''; }); }); }
function mountDemandAttachmentViewer() { const record = state.modal?.type === 'demand-details' ? state.modal.record : null, target = document.querySelector('.ticket-description'); if (!record?.anexoPrint?.data || !['image/png', 'image/jpeg', 'image/webp'].includes(record.anexoPrint.mime) || !target || document.querySelector('.demand-attachment')) return; const section = document.createElement('section'); section.className = 'demand-attachment'; section.innerHTML = `<h3>Anexo</h3><img class="mail-screenshot" alt="Print anexado à demanda" src="data:${record.anexoPrint.mime};base64,${record.anexoPrint.data}"/>`; target.after(section); }
new MutationObserver(() => { mountMailScreenshotFields(); mountDemandScreenshotFields(); mountDemandAttachmentViewer(); }).observe(document.getElementById('app'), { childList: true, subtree: true });
...
function mailThreadMessage(message, mine) { const screenshot = message.attachmentData && ['image/png', 'image/jpeg', 'image/webp'].includes(message.attachmentMime) ? `<img class="mail-screenshot" alt="Print anexado à mensagem" src="data:${message.attachmentMime};base64,${message.attachmentData}"/>` : ''; return `<article class="mail-thread-message ${message.sender.id === mine ? 'mine' : ''}"><header><span><b>${esc(message.sender.id === mine ? 'Você' : message.sender.nome)}</b>${message.sender.id === mine ? '<small>Enviado por você</small>' : ''}</span><time>${formatDate(message.createdAt)}</time></header>${message.body ? `<p>${esc(message.body).replace(/\n/g, '<br/>')}</p>` : ''}${screenshot}</article>`; }
...
async function sendMail(event, replyToId = '', currentThreadId = '') {
  ...
  try {
    ...
    if (currentThreadId) {
      ...
      const message = { ...result.message, sender: state.user, recipient, attachmentData: data.attachment?.data || null, attachmentMime: data.attachment?.mime || null };
