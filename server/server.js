const { materialRequestAccess, canViewMaterialRequest, sanitizeMaterialRequest } = require('./domain/material-requests');
...
function readFileDb() { if (!fs.existsSync(DB_FILE)) { fs.mkdirSync(DB_DIR, { recursive: true }); const data = initialData(); writeFileDb(data); return data; } const raw = fs.readFileSync(DB_FILE, 'utf8').replace(/^\uFEFF/, ''); return normalizeFileDb(encryptedStore.deserialize(raw)); }
...
async function migratePostgres() {
  await pool.query(`CREATE TABLE IF NOT EXISTS users (id UUID PRIMARY KEY, nome TEXT NOT NULL, email TEXT UNIQUE, login TEXT UNIQUE, setor TEXT NOT NULL DEFAULT '', cpf_hash TEXT UNIQUE, cpf_last4 TEXT NOT NULL DEFAULT '', data_nascimento DATE, perfil TEXT NOT NULL, active BOOLEAN NOT NULL DEFAULT true, activation_status TEXT NOT NULL DEFAULT 'ativo', permissions JSONB NOT NULL DEFAULT '{}'::jsonb, salt TEXT NOT NULL, hash TEXT NOT NULL, must_change_password BOOLEAN NOT NULL DEFAULT false, created_at TIMESTAMPTZ NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
    ...
    ALTER TABLE messages ADD COLUMN IF NOT EXISTS sender_archived_at TIMESTAMPTZ;
    ALTER TABLE messages ADD COLUMN IF NOT EXISTS recipient_archived_at TIMESTAMPTZ;
...
async function verifyPostgresSchema() {
  ...
  await pool.query('SELECT id,sender_id,recipient_id,subject,body,created_at,read_at,sender_deleted_at,recipient_deleted_at,sender_archived_at,recipient_archived_at FROM messages LIMIT 0');
...
async function api(req, res, url) {
  ...
  if (req.method === 'GET' && pathname === '/api/users') { if (user.perfil !== 'admin') return error(res, 403, 'Apenas administradores podem consultar usuários.'); return respond(res, 200, { users: (await store.users()).map(publicUser) }); }
  ...
  const preRegistrationMatch = pathname.match(/^\/api\/users\/([\w-]+)\/pre-cadastro$/); if (req.method === 'PUT' && preRegistrationMatch) { if (user.perfil !== 'admin') return error(res, 403, 'Apenas administradores podem corrigir pré-cadastros.'); const target = await store.findUser(preRegistrationMatch[1]); const body = await requestBody(req); const cpf = normalizeCpf(body.cpf), dataNascimento = String(body.dataNascimento || ''); if (!target || target.activationStatus !== 'pre-cadastro' || target.active !== false) return error(res, 422, 'Somente pré-cadastros inativos podem ser corrigidos.'); if (!cpf || !/^\d{4}-\d{2}-\d{2}$/.test(dataNascimento)) return error(res, 422, 'Informe CPF e data de nascimento corretamente.'); if ((await store.users()).some(item => item.id !== target.id && item.cpfHash === cpfHash(cpf))) return error(res, 409, 'Já existe um cadastro para este CPF.'); const updated = await store.updateUser(target.id, { cpfHash: cpfHash(cpf), cpfLast4: cpf.slice(-4), dataNascimento }); deleteChallengesForUser(firstAccessChallenges, updated.id); await log(user.id, 'corrigiu dados de validação do pré-cadastro', 'users', updated.id, { nome: updated.nome, setor: updated.setor || '' }); return respond(res, 200, { user: publicUser(updated) }); }
  const reopenFirstAccessMatch = pathname.match(/^\/api\/users\/([\w-]+)\/reopen-first-access$/); if (req.method === 'POST' && reopenFirstAccessMatch) { if (user.perfil !== 'admin') return error(res, 403, 'Apenas administradores podem reabrir o primeiro acesso.'); const target = await store.findUser(reopenFirstAccessMatch[1]); const incompleteCredentials = !String(target?.email || '').trim() && !String(target?.login || '').trim(); if (!target || target.id === user.id || target.active !== true || !incompleteCredentials || !target.cpfHash || !target.dataNascimento) return error(res, 422, 'Este cadastro não pode ser devolvido ao primeiro acesso.'); const placeholderCredentials = passwordHash(crypto.randomBytes(32).toString('hex')); const updated = await store.updateUser(target.id, { email: null, login: null, active: false, activationStatus: 'pre-cadastro', permissions: collaboratorPermissions(), ...placeholderCredentials, mustChangePassword: false }); for (const [token, session] of sessions) if (session.userId === updated.id) sessions.delete(token); deleteVerificationChallenges(updated.id); deleteChallengesForUser(firstAccessChallenges, updated.id); await log(user.id, 'reabriu primeiro acesso', 'users', updated.id, { nome: updated.nome, setor: updated.setor || '' }); return respond(res, 200, { user: publicUser(updated) }); }
  ...
  if (req.method === 'GET' && pathname === '/api/dashboard') {
    const assetResources = ['computadores', 'equipamentos', 'patrimonio'];
    const activeCount = (await Promise.all(assetResources.filter(resource => canAccess(user, resource)).map(async resource => (await store.records(resource)).length))).reduce((total, count) => total + count, 0);
    const demands = canAccess(user, 'demandas') ? (await store.records('demandas')).filter(record => demandBelongsToUser(record, user) && !isCompletedDemandStatus(record.status)) : []; const maintenance = (await Promise.all(['computadores', 'equipamentos', 'ramais', 'redes'].filter(resource => canAccess(user, resource)).map(resource => store.records(resource)))).flat().filter(item => /manutenção/i.test(item.status || item.condicao || item.funcionamento || '')).length; const inbox = (await store.messagesFor(user.id)).filter(message => message.recipientId === user.id && !message.readAt).length;
    ...
    return respond(res, 200, { activeCount, openDemands: demands.length, maintenance, inbox, notifications, announcements: await store.announcements(), demands: demands.slice(0, 4), syncedAt: now() });
  }
  ...
  if (req.method === 'GET' && pathname === '/api/messages') { const people = new Map((await store.users()).map(person => [person.id, person])); const messages = (await store.messagesFor(user.id)).sort((a, b) => b.createdAt.localeCompare(a.createdAt)).map(({ attachmentData, ...message }) => ({ ...message, hasAttachment: Boolean(attachmentData || message.attachmentMime), sender: publicUser(people.get(message.senderId)), recipient: publicUser(people.get(message.recipientId)) })); return respond(res, 200, { messages }); }
  const attachmentMatch = pathname.match(/^\/api\/messages\/([\w-]+)\/attachment$/); if (req.method === 'GET' && attachmentMatch) { const message = await store.messageFor(attachmentMatch[1], user.id); if (!message?.attachmentData || !message.attachmentMime) return error(res, 404, 'Print não encontrado.'); const image = Buffer.isBuffer(message.attachmentData) ? message.attachmentData : Buffer.from(message.attachmentData, 'base64'); res.writeHead(200, { 'content-type': message.attachmentMime, 'content-length': image.length, 'cache-control': 'private, no-store', 'x-content-type-options': 'nosniff', 'content-disposition': `inline; filename="${String(message.attachmentName || 'print').replace(/[^\w.-]/g, '_')}"` }); return res.end(image); }
  if (req.method === 'POST' && pathname === '/api/messages') {
    const { recipientId: requestedRecipientId, subject: requestedSubject, body, replyToId, attachment } = await requestBody(req, 7_000_000);
    const cleanBody = String(body || '').trim();
    const allowedImages = { 'image/png': ['png', '89504e470d0a1a0a'], 'image/jpeg': ['jpg', 'ffd8ff'], 'image/webp': ['webp', '52494646'] };
    let attachmentName = null, attachmentMime = null, attachmentData = null;
    if (attachment) { if (!allowedImages[attachment.mime] || typeof attachment.data !== 'string' || attachment.data.length > 6_700_000) return error(res, 422, 'Envie somente um print PNG, JPG ou WEBP de até 5 MB.'); const binary = Buffer.from(attachment.data, 'base64'); const [extension, signature] = allowedImages[attachment.mime]; if (!binary.length || binary.length > 5_000_000 || binary.subarray(0, signature.length / 2).toString('hex') !== signature) return error(res, 422, 'O print enviado é inválido.'); attachmentName = `${id()}.${extension}`; attachmentMime = attachment.mime; attachmentData = binary.toString('base64'); }
    if ((!cleanBody && !attachmentData) || cleanBody.length > 5000) return error(res, 422, 'Informe uma mensagem de até 5.000 caracteres ou adicione um print.');
    if (attachmentData) await assertAttachmentQuota(user.id, { data: attachmentData });
    ...
    const messageId = id();
    const message = { id: messageId, senderId: user.id, recipientId, threadId: threadId || messageId, replyToId: parent?.id || null, subject, body: cleanBody, attachmentName, attachmentMime, attachmentData, createdAt: now(), readAt: null };
    await store.createMessage(message);
    delete message.attachmentData;
  ...
  const archiveMessageMatch = pathname.match(/^\/api\/messages\/([\w-]+)\/archive$/); if (req.method === 'PUT' && archiveMessageMatch) { const archived = await store.archiveMessageFor(archiveMessageMatch[1], user.id); if (!archived) return error(res, 404, 'Mensagem não encontrada.'); await log(user.id, 'arquivou mensagem', 'messages', archiveMessageMatch[1]); return respond(res, 200, { ok: true }); }
  const restoreMessageMatch = pathname.match(/^\/api\/messages\/([\w-]+)\/restore$/); if (req.method === 'PUT' && restoreMessageMatch) { const restored = await store.restoreMessageFor(restoreMessageMatch[1], user.id); if (!restored) return error(res, 404, 'Mensagem não encontrada.'); await log(user.id, 'restaurou mensagem', 'messages', restoreMessageMatch[1]); return respond(res, 200, { ok: true }); }
  ...
  const exportMatch = pathname.match(/^\/api\/resources\/([a-z]+)\/export$/); if (req.method === 'GET' && exportMatch) { const resource = exportMatch[1]; if (!resourceDefinitions[resource] || DISABLED_RESOURCES.has(resource)) return error(res, 404, 'Recurso não encontrado.'); if (!canAccess(user, resource)) return error(res, 403, 'Você não tem permissão para esta área.'); const keys = resource === 'pedidos' ? [...resourceDefinitions.pedidos, 'observacoes', 'createdAt'] : resource === 'computadores' ? [...resourceDefinitions[resource], ...optionalResourceFields.computadores, 'dataSolicitacao', 'dataRetirada', 'dataDevolucao', 'checklist', 'observacoes'] : resource === 'materiais' ? [...resourceDefinitions[resource], 'observacoes'] : resourceDefinitions[resource]; let records = await store.records(resource); if (resource === 'pedidos') records = records.filter(record => canViewMaterialRequest(record, user)); if (resource === 'demandas') records = records.filter(record => demandBelongsToUser(record, user)); const content = csv(records, keys); res.writeHead(200, { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': `attachment; filename="central-ti-${resource}.csv"`, 'cache-control': 'no-store', 'x-content-type-options': 'nosniff' }); return res.end(content); }
  ...
  if (req.method === 'POST' && demandCommentMatch) {
    ...
    const { text = '', anexoPrint } = await requestBody(req, 7_000_000);
    const cleanText = repairTextEncoding(String(text).trim());
    const screenshot = sanitizeScreenshot(anexoPrint);
    if (screenshot === null) return error(res, 422, 'Envie somente um print PNG, JPG ou WEBP de até 5 MB.');
    if ((!cleanText && !screenshot) || cleanText.length > 3000) return error(res, 422, 'Escreva uma resposta ou anexe um print. O texto pode ter até 3.000 caracteres.');
    if (screenshot) await assertAttachmentQuota(user.id, screenshot);
    const interaction = { id: id(), texto: cleanText, autorId: user.id, criadoEm: now(), ...(screenshot ? { anexoPrint: { ...screenshot, ownerId: user.id } } : {}) };
    await store.updateRecord('demandas', demand.id, { interacoes: [...(demand.interacoes || []), interaction] }, user.id);
    await notifyTicketComment(demand, user, cleanText || 'Anexou um print para análise.');
  ...
  const match = pathname.match(/^\/api\/resources\/([a-z]+)(?:\/([\w-]+))?$/); if (match) {
    ...
    if (req.method === 'GET') { let records; if (recordId) { const record = await store.record(resource, recordId); records = record ? [record] : []; } else records = resource === 'demandas' && store.demandSummaries ? await store.demandSummaries() : await store.records(resource); if (resource === 'pedidos') records = records.filter(record => canViewMaterialRequest(record, user)); if (resource === 'demandas') records = records.filter(record => demandBelongsToUser(record, user)); if (resource === 'demandas') { const people = new Map((await store.users()).map(person => [person.id, person.nome])); records = records.map(record => { const response = demandForResponse(record, Boolean(recordId)); return { ...response, interacoes: response.interacoes.map(item => ({ ...item, autorNome: people.get(item.autorId) || 'Usuário removido' })) }; }); } if (recordId && !records.length) return error(res, 404, 'Registro não encontrado.'); return respond(res, 200, { records }); }
    if (req.method === 'POST' && !recordId) {
      const body = await requestBody(req, resource === 'demandas' ? 7_000_000 : 1_000_000); if (resource === 'patrimonio') body.codigo = await nextPatrimonyCode(); if (resource === 'demandas' && hospitalOnly(user) && body.tipo !== 'externa') return error(res, 403, 'Este usuário pode abrir somente demandas hospitalares.'); if (resource === 'demandas' && hospitalOnly(user)) { body.solicitante = user.nome; body.status = 'Aberta'; body.tecnicoResponsavel = ''; body.prazoSla = ''; } const payload = resource === 'demandas' ? (hospitalOnly(user) ? sanitizeHospitalDemand(body, user) : sanitizeDemand(body)) : resource === 'pedidos' ? sanitizeMaterialRequest(body, user) : sanitize(body, resourceDefinitions[resource]);
      ...
      if (resource === 'demandas') { const screenshot = sanitizeScreenshot(body.anexoPrint); if (screenshot === null) return error(res, 422, 'Envie somente um print PNG, JPG ou WEBP de até 5 MB.'); if (screenshot) { await assertAttachmentQuota(user.id, screenshot); payload.anexoPrint = { ...screenshot, ownerId: user.id }; } }
