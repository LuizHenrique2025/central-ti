...
const pgStore = {
  ...
  async messagesFor(userId) { return (await pool.query('SELECT id,sender_id AS "senderId",recipient_id AS "recipientId",thread_id AS "threadId",reply_to_id AS "replyToId",subject,body,created_at AS "createdAt",read_at AS "readAt",sender_deleted_at AS "senderDeletedAt",recipient_deleted_at AS "recipientDeletedAt",sender_archived_at AS "senderArchivedAt",recipient_archived_at AS "recipientArchivedAt" FROM messages WHERE recipient_id=$1 OR sender_id=$1 ORDER BY created_at DESC', [userId])).rows.map(normalizePgDates); },
