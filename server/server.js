...
function demandForResponse(record, includeAttachment = false) { const value = { ...record }; if (!includeAttachment && value.anexoPrint?.data) value.anexoPrint = { mime: value.anexoPrint.mime, hasAttachment: true }; return value; }
function auditSafeRecord(record) { const value = { ...record }; if (value.anexoPrint?.data) value.anexoPrint = { mime: value.anexoPrint.mime, hasAttachment: true }; return value; }
...
const fileStore = {
  ...
  async attachmentUsageFor(userId) { const db = readFileDb(); const messages = db.messages.filter(message => message.senderId === userId && message.attachmentData); const demands = db.demandas.filter(record => record.anexoPrint?.data && (record.anexoPrint.ownerId || record.createdBy) === userId); return { count: messages.length + demands.length, bytes: messages.reduce((total, message) => total + Buffer.byteLength(message.attachmentData, 'base64'), 0) + demands.reduce((total, record) => total + attachmentBytes(record.anexoPrint), 0) }; },
...
async function api(req, res, url) {
  ...
  const match = pathname.match(/^\/api\/resources\/([a-z]+)(?:\/([\w-]+))?$/); if (match) {
    ...
    if (req.method === 'GET') { let records = await store.records(resource); if (recordId) records = records.filter(record => record.id === recordId); if (resource === 'demandas') records = records.filter(record => demandBelongsToUser(record, user)); if (resource === 'demandas') { const people = new Map((await store.users()).map(person => [person.id, person.nome])); records = records.map(record => ({ ...demandForResponse(record, Boolean(recordId)), interacoes: (record.interacoes || []).map(item => ({ ...item, autorNome: people.get(item.autorId) || 'Usuário removido' })) })); } if (recordId && !records.length) return error(res, 404, 'Registro não encontrado.'); return respond(res, 200, { records }); }
