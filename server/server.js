...
async function api(req, res, url) {
  ...
  const passwordMatch = pathname.match(/^\/api\/users\/([\w-]+)\/password$/); if (req.method === 'PUT' && passwordMatch) { if (user.perfil !== 'admin') return error(res, 403, 'Apenas administradores podem redefinir senhas.'); if (passwordMatch[1] === user.id) return error(res, 422, 'Use a alteração de senha do seu próprio perfil.'); const { password } = await requestBody(req); if (typeof password !== 'string' || password.length < 8 || !/[a-z]/.test(password) || !/[A-Z]/.test(password) || !/\d/.test(password) || !/[^A-Za-z0-9]/.test(password)) return error(res, 422, 'Use ao menos 8 caracteres, com maiúscula, minúscula, número e símbolo.'); const updated = await store.updatePassword(passwordMatch[1], password, true); if (!updated) return error(res, 404, 'Usuário não encontrado.'); for (const [token, session] of sessions) if (session.userId === updated.id) sessions.delete(token); await log(user.id, 'redefiniu senha', 'users', updated.id, { nome: updated.nome, trocaObrigatoria: true }); return respond(res, 200, { ok: true }); }
